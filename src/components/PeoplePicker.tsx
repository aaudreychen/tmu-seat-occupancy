...
export function PeoplePicker({ selectedPeople, onSelectPeople, onClose }: PeoplePickerProps) {
  const peopleOptions = ['1 Person', '2 People', '3 People', '4 People', 'More than 4'];
      ...
      {peopleOptions.map((option) => (
        <button
          ...
          className={`w-full px-4 py-2.5 text-left hover:bg-gray-100 transition-colors ${
            selectedPeople === option ? 'bg-blue-50 text-blue-600 font-medium' : 'text-gray-900'
          }`}
        >
          {option}
        </button>
